    ...
    _renderBook(book){
      return(
          <div className="checkbox" key={book.id}>
              <label>
                  <input type="checkbox" value={book.name} onChange={this.handleSelectedBook}/> {book.name} -- {book.author}
              </label>
          </div>
      );
    },
...
const Success = React.createClass({
    render(){
        let numberOfDays = "1 of 2";

        if(this.props.data.deliveryOption === 'Option'){
            numberOfDays = "3 of 4";
        }
...
const BookStore = React.createClass({
    ...
    updateFormData(formData){
      console.log(`BookStore com: ${formData}`);
